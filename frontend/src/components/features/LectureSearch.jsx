...
const LectureSearch = ({ isGoalMode }) => {
  ...
  const handleSearch = async (e, queryOverride = null) => {
    ...
    setIsLoading(true);
    setResults([]);
    const videoData = await searchYoutubeVideos(query, 6); 
  ...
  return (
    <div className={`${containerClass} rounded-xl p-3 border text-center h-full flex flex-col justify-between transition-all duration-500`}>
      <div className={`flex items-center gap-2 mb-2 border-b pb-1 shrink-0 ${isGoalMode ? 'border-gray-800' : 'border-gray-900'}`}>
        <h3 className={`font-bold text-xs tracking-wide ${textClass}`}>LECTURE SEARCH</h3>
      ...
      <div className="flex-1 overflow-y-auto px-1 custom-scrollbar">
        {isLoading ? <p className={`text-center text-[10px] py-4 ${subTextClass}`}>강의 검색 중...</p> : results.length > 0 ? (
          ...
          <div className="pt-2">
            <p className={`text-[10px] font-medium mb-2 ${subTextClass}`}>필요한 강의를 찾아보세요</p>
